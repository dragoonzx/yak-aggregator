...
describe("Yak Router - swap", () => {
  ...
  it("Expect best amount from all adapters & tokens to be equal on-chain swap amount out result", async () => {
    // Call the query
    ...
    let adapters = await YakRouter.favoriteAdaptersCount();
    expect(adapters.toNumber()).to.equal(3);
    ...
    // Do the swap
    await YakRouter.connect(trader).findPathAndSwap(
      amountIn,
      tokenIn,
      tokenOut,
      trader.address,
    ...
    outputTokenContract = await ethers.getContractAt(
  ...
  it("Router swap matched the query - with permit", async () => {
    ...
    const chainId = 43114;
    // Permit setting
    let inputTokenContract = await ethers.getContractAt(
      "contracts/interface/IERC20.sol:IERC20",
      tokenIn
    );
  ...
  it("Mix swap can be done to AVAX", async () => {
    ...
    await expect(await swap).to.changeEtherBalance(
      trader,
      result.amounts[result.amounts.length - 1]
    );
  });
  ...
  it("Mix swap can be done from AVAX", async () => {
    ...
    let tokenOutBal = await helpers.getERC20Balance(
      ethers.provider,
      tokenOut,
      trader.address
    );
  ...
  it("Transactions reverts if expected out-amount is not within slippage", async () => {
    ...
    await tokenIn
      ...
      .transfer(firstAdapter.address, amountInChange);
    const amountOut = await firstAdapter.query(
      amountInChange,
      tokenIn.address,
      tokenOut.address
    );
  ...
  it("User gets expected out-amount+positiveSlippage if conditions change positively", async () => {
    ...
    let firstAdapter = Object.values(adapters).find(
      (a) => a.address == result.adapters[0]
    );
  ...
  it("Swap with 3-4 steps", async () => {
    ...
    // Top up trader with starting tokens
    let [topUpAmountIn] = await fix.ZeroRouter.getAmountsIn(amountIn, [
      assets.WAVAX,
      assets.ZERO,
      assets.ZUSDT,
    ]);
