...
const _curveAdapter = async () => {
  ...
  const CurvePlainAdapterFactory =
    ethers.getContractFactory("CurvePlainAdapter");
  ...
  const [
    ...
    Curve2AdaptorFactory.then((f) =>
      f.deploy("CurveAaveAdapter", curvelikePools.CurveAave, 9.1e5)
    ),
    Curve1AdaptorFactory.then((f) =>
      f.deploy(
        "CurveAtricryptoAdapter",
    ...
    Curve2AdaptorFactory.then((f) =>
      f.deploy("CurveRenAdapter", curvelikePools.CurveRen, 6.3e5)
    ),
    CurvePlainAdapterFactory.then((f) =>
      f.deploy("Curve3poolV2Adapter", curvelikePools.Curve3poolV2, 2.9e5)
    ),
...
const _platypusAdapter = async () => {
  ...
  const [PlatypusV1, PlatypusAdapter] = await Promise.all([
    ...
    PlatypusAdapterFactory.then((f) =>
      f.deploy("PlatypusV1Adapter", curvelikePools.PlatypusV1, 5.14e5)
    ),
  ]);
  return {
    PlatypusAdapter,
...
const _axialAdapter = async () => {
  const CurvelikeMetaAdapterFactory = ethers.getContractFactory(
    "CurvelikeMetaAdapter"
  );
  ...
  const [
    ...
    CurvelikeMetaAdapterFactory.then((f) =>
      f.deploy("AxialAM3DUSDCAdapter", curvelikePools.AxialAM3DUSDC, 6.5e5)
    ),
    CurveLikeAdapterFactory.then((f) =>
      f.deploy("AxialAM3DAdapter", curvelikePools.AxialAM3D, 3.6e5)
    ),
    CurveLikeAdapterFactory.then((f) =>
      f.deploy("AxialAC4DAdapter", curvelikePools.AxialAC4D, 3.6e5)
    ),
    CurveLikeAdapterFactory.then((f) =>
      f.deploy("AxialAA3DAdapter", curvelikePools.AxialAA3D, 3.6e5)
    ),
    CurveLikeAdapterFactory.then((f) =>
      f.deploy("AxialAS4DAdapter", curvelikePools.AxialAS4D, 3.6e5)
    ),
  ]);
  return {
    AxialAM3DUSDC,
...
const _curvelikeAdapters = async () => {
  ...
  const pools = {
    GondolaUSDTUSDTe: await ethers.getContractAt(
      "ICurveLikePool",
      curvelikePools.GondolaUSDTUSDTe
    ),
    GondolaUSDTeUSDCe: await ethers.getContractAt(
      "ICurveLikePool",
      curvelikePools.GondolaUSDTeUSDCe
    ),
    GondolaUSDTeTSD: await ethers.getContractAt(
      "ICurveLikePool",
      curvelikePools.GondolaUSDTeTSD
    ),
    GondolaUSDT: await ethers.getContractAt(
      "ICurveLikePool",
      curvelikePools.GondolaUSDT
    ),
    GondolaDAI: await ethers.getContractAt(
      "ICurveLikePool",
      curvelikePools.GondolaDAI
    ),
    GondolaBTC: await ethers.getContractAt(
      "ICurveLikePool",
      curvelikePools.GondolaBTC
    ),
    GondolaETH: await ethers.getContractAt(
      "ICurveLikePool",
      curvelikePools.GondolaETH
    ),
    SnobF3D: await ethers.getContractAt(
      "ICurveLikePool",
      curvelikePools.snobF3D
    ),
    SnobS3D: await ethers.getContractAt(
      "ICurveLikePool",
      curvelikePools.snobS3D
    ),
    SnobS4D: await ethers.getContractAt(
      "ICurveLikePool",
  ...
  // Init Adapters
  const adapters = {};
  const CurveLikeAdapterFactory = await ethers.getContractFactory(
    "CurveLikeAdapter"
  );
  adapters["SnobF3DAdapter"] = await CurveLikeAdapterFactory.connect(
    deployer
  ).deploy("SnobF3D YakAdapter", curvelikePools.snobF3D, 170000);
  adapters["SnobS3DAdapter"] = await CurveLikeAdapterFactory.connect(
    deployer
  ).deploy("SnobS3D YakAdapter", curvelikePools.snobS3D, 170000);
  adapters["SnobS4DAdapter"] = await CurveLikeAdapterFactory.connect(
    deployer
  ).deploy("SnobS4D YakAdapter", curvelikePools.snobS4D, 180000);
  adapters["GondolaDAIAdapter"] = await CurveLikeAdapterFactory.connect(
    deployer
  ).deploy("GondolaDAI YakAdapter", curvelikePools.GondolaDAI, 150000);
  adapters["GondolaBTCAdapter"] = await CurveLikeAdapterFactory.connect(
    deployer
  ).deploy("GondolaBTC YakAdapter", curvelikePools.GondolaBTC, 150000);
  adapters["GondolaUSDTAdapter"] = await CurveLikeAdapterFactory.connect(
    deployer
  ).deploy("GondolaUSDT YakAdapter", curvelikePools.GondolaUSDT, 150000);
  adapters["GondolaETHAdapter"] = await CurveLikeAdapterFactory.connect(
    deployer
  ).deploy("GondolaETH YakAdapter", curvelikePools.GondolaETH, 150000);
  adapters["GondolaUSDTeUSDCeAdapter"] = await CurveLikeAdapterFactory.connect(
    deployer
  ).deploy(
  ...
  adapters["GondolaUSDTeTSDAdapter"] = await CurveLikeAdapterFactory.connect(
    deployer
  ).deploy(
  ...
  adapters["GondolaUSDTUSDTeAdapter"] = await CurveLikeAdapterFactory.connect(
    deployer
  ).deploy(
...
const _unilikeAdapters = async () => {
  const [deployer] = await ethers.getSigners();
  const adapters = {};
  const UnilikeAdapterFactory = await ethers.getContractFactory(
    "UnilikeAdapter"
  );
  ...
  adapters["PandaAdapter"] = await UnilikeAdapterFactory.connect(
    deployer
  ).deploy("PandaSwap YakAdapter", unilikeFactories.pandaswap, 3, 100000);
  adapters["OliveAdapter"] = await UnilikeAdapterFactory.connect(
    deployer
  ).deploy("OliveSwap YakAdapter", unilikeFactories.olive, 2, 100000);
  adapters["SushiswapAdapter"] = await UnilikeAdapterFactory.connect(
    deployer
  ).deploy("Sushiswap YakAdapter", unilikeFactories.sushiswap, 3, 100000);
  adapters["ComplusAdapter"] = await UnilikeAdapterFactory.connect(
    deployer
  ).deploy("Complus YakAdapter", unilikeFactories.complus, 3, 100000);
  adapters["ZeroAdapter"] = await UnilikeAdapterFactory.connect(
    deployer
  ).deploy("Zero YakAdapter", unilikeFactories.zero, 3, 100000);
  adapters["PangolinAdapter"] = await UnilikeAdapterFactory.connect(
    deployer
  ).deploy("Pangolin YakAdapter", unilikeFactories.pangolin, 3, 100000);
  adapters["LydiaAdapter"] = await UnilikeAdapterFactory.connect(
    deployer
  ).deploy("Lydia YakAdapter", unilikeFactories.lydia, 2, 100000);
  adapters["YetiAdapter"] = await UnilikeAdapterFactory.connect(
    deployer
  ).deploy("Yeti YakAdapter", unilikeFactories.yeti, 3, 100000);
...
const _bridgeMigrationAdapters = async () => {
  ...
  const adapterFactory = await ethers.getContractFactory(
    "BridgeMigrationAdapter"
  );
  const oldTokens = [
    await ethers
      .getContractFactory("TestToken")
      .then((f) => f.deploy("TToken1", "TT1")),
    await ethers
      .getContractFactory("TestToken")
      .then((f) => f.deploy("TToken2", "TT2")),
  ];
  const bridgeTokens = [
    await ethers
      .getContractFactory("BridgeToken")
      .then((f) => f.deploy("BToken1", "BT1")),
    await ethers
      .getContractFactory("BridgeToken")
      .then((f) => f.deploy("BToken1", "BT1")),
  ];
...
const simple = deployments.createFixture(async () => {
  ...
  const tokenContracts = await Promise.all(
    Object.keys(assets).map((tknSymbol) => {
      return helpers
        .getTokenContract(assets[tknSymbol])
        .then((tc) => [tknSymbol, tc]);
    })
...
const general = deployments.createFixture(async () => {
  ...
  for (tknSymbol of Object.keys(assets)) {
    tokenContracts[tknSymbol] = await helpers.getTokenContract(
      assets[tknSymbol]
    );
  }
  // Get accounts
  ...
  const PangolinRouter = await ethers.getContractAt(
    "IUnilikeAVAXRouter",
    unilikeRouters.pangolin
  );
  const LydiaRouter = await ethers.getContractAt(
    "IUnilikeAVAXRouter",
    unilikeRouters.lydia
  );
  const SushiswapRouter = await ethers.getContractAt(
    "IUnilikeETHRouter",
    unilikeRouters.sushiswap
  );
  const ZeroRouter = await ethers.getContractAt(
    "IUnilikeETHRouter",
    unilikeRouters.zero
  );
  // Set tags
...
const router = deployments.createFixture(async ({}) => {
  ...
  const BytesManipulationFactory = await ethers.getContractFactory(
    "BytesManipulation"
  );
  ...
  const favoriteAdapters = [
    adapters.PangolinAdapter.address,
    adapters.LydiaAdapter.address,
    adapters.JoeAdapter.address,
  ];
  await YakRouter.connect(deployer).setFavoriteAdapters(favoriteAdapters);
  // Set tags
  if (TRACER_ENABLED) {
    hre.tracer.nameTags["deployer"] = deployer;
